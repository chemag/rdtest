...
def run_experiment(options):
    ...
    assert ref_resolution == get_resolution(
        ref_filename
    ), "Error: %s must have resolution: %s (is %s)" % (
        ref_filename,
        ref_resolution,
        get_resolution(ref_filename),
    )
    assert ref_pix_fmt == get_pix_fmt(
        ref_filename
    ), "Error: %s must have pix_fmt: %s (is %s)" % (
        ref_filename,
        ref_pix_fmt,
        get_pix_fmt(ref_filename),
    )
